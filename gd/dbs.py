...
from gd.obj import Object, search, update
from gd.obj import load, save
from gd.utl import locked
...
class Db(Object):
    ...
    @staticmethod
    def types():
        assert gd.obj.workdir
        path = os.path.join(gd.obj.workdir, "store")
...
def fns(name, timed=None):
    if not name:
        return []
    assert gd.obj.workdir
    p = os.path.join(gd.obj.workdir, "store", name) + os.sep
...
def all(timed=None):
    assert gd.obj.workdir
    p = os.path.join(gd.obj.workdir, "store")
