...
def printable(oobj, args="", skip="_", empty=False, plain=False, **kwargs):
    keyz = list(keys(oobj))
    res = []
    if args:
        try:
...
def load(oobj, opath):
    ...
    lpath = os.path.join(Cfg.workdir, "store", stp)
...
def save(oobj, stime=None):
    ...
    oobj.__stp__ = os.path.join(prv,
                             os.sep.join(str(datetime.datetime.now()).split()))
    opath = os.path.join(Cfg.workdir, "store", oobj.__stp__)
    dump(oobj, opath)
...
def fns(name, timed=None):
    if not name:
        return []
    assert Cfg.workdir
    path = os.path.join(Cfg.workdir, "store", name) + os.sep
    if not os.path.exists(path):
        return []
    res = []
    dpath = ""
